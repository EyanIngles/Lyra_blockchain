...
impl Token {
    #[allow(dead_code)] //TODO:: working progress
    pub fn new() -> Token {
    ...
    #[allow(dead_code)]
    fn token_standard(token: TokenStd) -> bool {
        // Check if token being created is within standard or not which will pass back a boolean value.
        TokenStd::is_following_token_std(token)
    }
    #[allow(unused_variables, dead_code)]
    pub fn mint_tokens(token: &mut Token, mint_amount: u64, owner: String) {
        //owner is passing a string currently until addresses are correctly implemented.
        // need to check owner against the wallet doing the transaction.
        if !TokenStd::get_has_totsup(&token.std) {
...
#[test]
pub fn test_new_token_is_standard_approved() {
    //This is a temporary test - more logic required
